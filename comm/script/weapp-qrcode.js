...
!function() {
    ...
    }).prototype.makeCode = function(t, r) {
        this._oQRCode = new n(e(t, this._htOption.correctLevel), this._htOption.correctLevel), 
        this._oQRCode.addData(t), this._oQRCode.make(), this.makeImage(r);
    }, t.prototype.makeImage = function(t) {
        var e;
        e = this._htOption.usingIn ? wx.createCanvasContext(this.canvasId, this._htOption.usingIn) : wx.createCanvasContext(this.canvasId);
        var r = this._htOption, o = this._oQRCode, n = o.getModuleCount(), i = r.width / n, a = r.height / n, s = Math.round(i), h = Math.round(a);
        r.image && "" != r.image && e.drawImage(r.image, 0, 0, r.width, r.height);
        for (var u = 0; u < n; u++) for (var l = 0; l < n; l++) {
            var g = o.isDark(u, l), f = l * i, c = u * a;
            e.setStrokeStyle(g ? r.colorDark : r.colorLight), e.setLineWidth(1), e.setFillStyle(g ? r.colorDark : r.colorLight), 
            e.fillRect(f, c, i, a), e.strokeRect(Math.floor(f) + .5, Math.floor(c) + .5, s, h), 
            e.strokeRect(Math.ceil(f) - .5, Math.ceil(c) - .5, s, h);
        }
        e.draw(!1, t);
