...
Page({
  data: {
    ...
    sysInfo: getApp().globalData.systemInfo,
    defaultClassIndex: 0,
    tapIndex: 0,
    leftIndex: 0,
    menuVoList: [],
  ...
  onLoad: function (options) {
    ...
    this.shopcartBottom = shopcartBottom('.promotion', this)
  },
  ...
  order: function () {
    ...
    fetch('api.do?param=order', {
      order: this.data.cartList
    }, 'POST').then(data => {
      wx.navigateTo({
        url: '/pages/settlement/settlement?order_id=' + data.order_id
      })
      // console.log(data)
    }, () => {
      this.order()
...
function shopcartBottom(iconClass, page) {
  console.log('11')
  wx.createSelectorQuery().select(iconClass).boundingClientRect(rect => {
    console.log(rect)
    var bottom = page.data.sysInfo.windowHeight - rect.top
    page.setData({
      'shopCart.bottom': bottom
    })
  }).exec()
}
